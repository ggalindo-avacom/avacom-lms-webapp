...
function ProgressView({ language, role }) {
  ...
  return (
    <>
      ...
      <div className="module-grid"><ModuleCard title={localize(language, 'Mis notas por asignatura', 'My grades by subject')}><ModuleInfoRow label="Álgebra 8°B · Periodo 1" value="8.2" /><ModuleInfoRow label="Álgebra 8°B · Periodo 2" value="8.5" /></ModuleCard><ModuleCard title={localize(language, 'Mi rendimiento general', 'My overall performance')}><ModuleProgress value={85} /><p className="module-subtitle">{localize(language, 'Vas muy bien, Samuel: 85% de logro acumulado.', 'Great work, Samuel: 85% cumulative achievement.')}</p></ModuleCard></div>
...
function StudentsView({ activeTab, language, role, setActiveTab }) {
  ...
  else if (activeTab === 1) content = <ModuleCard title={tabs[1]}><Item detail={localize(language, '“Profe, ¿la tarea se puede entregar en foto?”', '“Can the homework be submitted as a photo?”')} action={<ModuleChip tone="warn">{localize(language, 'Sin responder', 'Unanswered')}</ModuleChip>}>Samuel Rodríguez</Item><Item detail={localize(language, '“¿El taller grupal cuenta para la nota?”', '“Does the group workshop count toward the grade?”')} action={<ModuleChip tone="ok">{localize(language, 'Respondida', 'Answered')}</ModuleChip>}>Isabella Cano</Item></ModuleCard>
